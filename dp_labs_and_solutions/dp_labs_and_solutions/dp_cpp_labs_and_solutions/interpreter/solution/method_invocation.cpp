...
double method_invocation::get_value(const dp_map<std::string, double> & a_map) const
{
	double result(0);
	...
	if (get_name() == "sqrt")
	{
		...
		{
			throw dp_exception("double method_invocation::get_value(dp_map<std::string, double> & a_map) const -- a_operand1 == nullptr.");
		}
		double a_value1 = a_operand1->get_value(a_map);

		result = std::sqrt(a_value1);
	}
	else
	{
		throw dp_exception("double method_invocation::get_value(dp_map<std::string, double> & a_map) const -- unsupported method name.");
	}
