...
double binary_minus::get_value(const dp_map<std::string, double> & a_map) const
{
	...
	{
		throw dp_exception("double binary_minus::get_value(dp_map<std::string, double> & a_map) const -- get_operand1() == nullptr.");
	}
	double a_value1 = get_operand1()->get_value(a_map);

	if (get_operand2() == nullptr)
	{
		throw dp_exception("double binary_minus::get_value(dp_map<std::string, double> & a_map) const -- get_operand2() == nullptr.");
	}
