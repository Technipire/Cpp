...
// D P _ L I S T _ R E V E R S E _ I T E R A T O R

template <typename T>
class dp_list_reverse_iterator : public dp_iterator<T>
{
public:
	dp_list_reverse_iterator(dp_list<T> & a_list);
	dp_list_reverse_iterator(const dp_list_reverse_iterator & other);
	dp_list_reverse_iterator & operator=(const dp_list_reverse_iterator & other);
	virtual ~dp_list_reverse_iterator();
...
private:
	...
	friend class dp_list_reverse_iterator<T>;
};
...
{
	return (my_current_link == nullptr);
}
...
template <typename T>
inline
dp_list_reverse_iterator<T>::dp_list_reverse_iterator(dp_list<T> & a_list)
{
	my_original_link = a_list.my_first;
	my_current_link  = a_list.my_last;
}

template <typename T>
inline
dp_list_reverse_iterator<T>::dp_list_reverse_iterator(const dp_list_reverse_iterator<T> & other)
{
	throw dp_exception("dp_list_reverse_iterator<T>::dp_list_reverse_iterator(const dp_list_reverse_iterator<T> & other) -- is not supported.");
}

template <typename T>
inline
dp_list_reverse_iterator<T> & dp_list_reverse_iterator<T>::operator=(const dp_list_reverse_iterator<T> & other)
{
	throw dp_exception("dp_list_reverse_iterator<T> & dp_list_reverse_iterator<T>::operator=(const dp_list_reverse_iterator<T> & other) -- is not supported.");
}

template <typename T>
inline
dp_list_reverse_iterator<T>::~dp_list_reverse_iterator()
{
}

template <typename T>
inline
bool dp_list_reverse_iterator<T>::is_done() const
{
	return (my_current_link == nullptr);
}
...
template <typename T>
inline
T & dp_list_reverse_iterator<T>::get_current() const
{
	return my_current_link->get_value();
}
...
template <typename T>
inline
void dp_list_reverse_iterator<T>::advance()
{
	if (my_current_link == my_original_link)
	{
		my_current_link = nullptr;
	}
	else
	{
		dp_link<T> * a_link = my_original_link;
		while (a_link->get_next() != my_current_link)
		{
			a_link = a_link->get_next();
		}
		my_current_link = a_link;
	}
}
...
template <typename T>
inline
dp_iterator<T> * dp_list<T>::make_reverse_iterator()
{
	return new dp_list_reverse_iterator<T>(*this);
}
